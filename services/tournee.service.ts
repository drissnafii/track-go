...
export const tourneeService = {
  ...
  getColisById: (id: string): Promise<Colis> =>
    apiClient.get<Colis>(`${Endpoints.COLIS}/${id}`),
  ...
  /** Report an incident for a colis */
  reportIncident: (incident: Incident): Promise<Incident> =>
    apiClient.post<Incident>(Endpoints.INCIDENTS, incident),

  /** Update colis status */
  updateColisStatus: (id: string, status: ColisStatus): Promise<Colis> =>
    apiClient.patch<Colis>(`${Endpoints.COLIS}/${id}`, { status }),
